...
function v310HasValue_(v) { return v !== '' && v !== null && v !== undefined && !isNaN(Number(v)); }
  ...
  var RECON_DISPATCHER_STATE_KEY='AKORT_ALPHA6_RECONCILIATION_DISPATCHER_STATE_V1';
  var RECON_DISPATCHER_HANDLER='AKORT_alpha6ReconciliationDispatcherWorker';
  var RECON_DISPATCHER_INITIAL_DELAY_MS=15000;
  var RECON_DISPATCHER_NORMAL_DELAY_MS=60000;
  var RECON_DISPATCHER_RETRY_DELAY_MS=180000;
  var RECON_DISPATCHER_WATCHDOG_DELAY_MS=420000;
  ...
  function advanceReplayStage_(state,groups,group,stage){
    ...
    else if(stage==='INDUSTRY')state.replayStage='AGGREGATES';
    else{
      markReplayGroup_(state,state.replayIndex,'SUCCESS');state.replayIndex+=1;state.replayStage='PREPARE';
      if(state.replayIndex>=groups.length)state.replayStage='FINALIZE_LATEST';
    }
  }
  ...
  function replayStep_(state){
    ...
    if(state.replayIndex>=groups.length){
      if(stage==='FINALIZE_LATEST'){var latestRows=replayFinalizeAggregateLatest_(state);state.replayStage='COMPLETE';state.replayWork=null;state.phase='NORMALIZE_WEEKLY';return{completed:true,stage:'FINALIZE_LATEST',aggregateRows:latestRows,groups:groups.length};}
      state.phase='NORMALIZE_WEEKLY';return{completed:true,groups:groups.length};
    }
  ...
  function continueReconciliation(){return AKORT.Core.safeRun('PUBLISH_RECONCILIATION_CONTINUE',function(){
    ...
    if(s.phase==='BUILD_FULL_WEEKLY'){
      ...
      if(industryActual===industryExpected&&industryActual>0){s.phase='BUILD_FULL_AGGREGATES';stepResult={recovered:true,sheet:AKORT_V300.SHEETS.PUBLISH_INDUSTRY,rows:industryActual};}else{writeBuildSheet_(s.fullId,AKORT_V300.SHEETS.PUBLISH_INDUSTRY,AKORT_V300.HEADERS.PUBLISH_INDUSTRY,v300BuildIndustryPublish_(),null);s.phase='BUILD_FULL_AGGREGATES';}
    }else if(s.phase==='BUILD_FULL_AGGREGATES')stepResult=reconAggregateBuildStep_(s);
    ...
    else if(s.phase==='NORMALIZE_INDUSTRY')stepResult=reconNormalizeStep_(s,AKORT_V300.SHEETS.PUBLISH_INDUSTRY,AKORT_V300.HEADERS.PUBLISH_INDUSTRY,'NORMALIZE_AGGREGATES');
    else if(s.phase==='NORMALIZE_AGGREGATES')stepResult=reconNormalizeStep_(s,AKORT_V300.SHEETS.PUBLISH_PRICE_AGGREGATES,AKORT_V300.HEADERS.PUBLISH_PRICE_AGGREGATES,'COMPARE_WEEKLY');
    else if(s.phase.indexOf('COMPARE_')===0)stepResult=reconCompareStep_(s);
    else if(s.phase==='FINALIZE'){var ok=(s.results||[]).length===4&&(s.results||[]).every(function(r){return r.status==='PASS';});s.status=ok?'SUCCESS':'FAILED';s.phase=ok?'SUCCESS':'COMPARE_FAILED';s.finishedAt=AKORT.Core.now();}
    ...
    return s.status==='FAILED'?AKORT.Result.failure('PUBLISH_RECONCILIATION_FAILED','Full, sequential incremental and baseline results do not match.',data):AKORT.Result.success(s.status==='SUCCESS'?'Alpha.6 triple reconciliation passed.':'Alpha.6 reconciliation step completed.',data);
  ...
  function dispatcherLoad_(){var raw=PropertiesService.getScriptProperties().getProperty(RECON_DISPATCHER_STATE_KEY);return raw?JSON.parse(raw):null;}
  function dispatcherSave_(state){state.updatedAt=AKORT.Core.now();PropertiesService.getScriptProperties().setProperty(RECON_DISPATCHER_STATE_KEY,JSON.stringify(state));return state;}
  ...
  function dispatcherSchedule_(delayMs){var delay=Math.max(1000,Number(delayMs||RECON_DISPATCHER_NORMAL_DELAY_MS)),trigger=ScriptApp.newTrigger(RECON_DISPATCHER_HANDLER).timeBased().after(delay).create();return{triggerId:(typeof trigger.getUniqueId==='function'?trigger.getUniqueId():''),delayMs:delay,scheduledFor:new Date(Date.now()+delay).toISOString()};}
  function dispatcherResultSummary_(result){var data=result&&result.data?result.data:{},replay=data.replay||{};return{ok:result?result.ok:false,status:result?result.status:'',message:result?result.message:'',reconciliationStatus:data.status||'',phase:data.phase||'',currentIndex:Number(replay.currentIndex||0),currentStage:replay.currentStage||'',currentLoadId:replay.currentLoadId||'',chunkCursor:Number(replay.chunkCursor||0),chunkTotal:Number(replay.chunkTotal||0),timestamp:result&&result.timestamp?result.timestamp:AKORT.Core.now()};}
  function dispatcherPublic_(state){var s=state||dispatcherLoad_()||{},recon=loadRecon_();return{enabled:s.enabled===true,status:s.status||'STOPPED',handler:RECON_DISPATCHER_HANDLER,triggerCount:dispatcherTriggers_().length,startedAt:s.startedAt||'',updatedAt:s.updatedAt||'',lastStartedAt:s.lastStartedAt||'',lastFinishedAt:s.lastFinishedAt||'',nextRunAt:s.nextRunAt||'',attempts:Number(s.attempts||0),successfulSteps:Number(s.successfulSteps||0),consecutiveErrors:Number(s.consecutiveErrors||0),interruptedRuns:Number(s.interruptedRuns||0),lastError:s.lastError||null,lastResult:s.lastResult||null,reconciliation:publicRecon_(recon)};}
  function dispatcherTerminalState_(state,recon,reason){dispatcherDeleteTriggers_();state.enabled=false;state.status=recon&&(recon.status==='SUCCESS'||recon.status==='FAILED')?recon.status:(reason||'STOPPED');state.nextRunAt='';state.lastFinishedAt=AKORT.Core.now();state.stopReason=reason||state.status;dispatcherSave_(state);return dispatcherPublic_(state);}
  function startReconciliationDispatcher(){return AKORT.Core.safeRun('PUBLISH_RECONCILIATION_DISPATCHER_START',function(){AKORT.EnvironmentGuard.assertDev();var recon=loadRecon_();if(!recon)return AKORT.Result.failure('RECONCILIATION_NOT_STARTED','Run AKORT_alpha6StartReconciliation first.');if(recon.status==='SUCCESS')return AKORT.Result.success('Reconciliation is already complete.',{dispatcher:dispatcherPublic_(dispatcherLoad_()),reconciliation:publicRecon_(recon)});if(recon.status==='FAILED')return AKORT.Result.failure('PUBLISH_RECONCILIATION_FAILED','Reconciliation is in failed status.',publicRecon_(recon));var previous=dispatcherLoad_()||{},removed=dispatcherDeleteTriggers_(),schedule=dispatcherSchedule_(RECON_DISPATCHER_INITIAL_DELAY_MS),state={enabled:true,status:'SCHEDULED',startedAt:previous.startedAt||AKORT.Core.now(),updatedAt:AKORT.Core.now(),lastStartedAt:previous.lastStartedAt||'',lastFinishedAt:previous.lastFinishedAt||'',nextRunAt:schedule.scheduledFor,triggerId:schedule.triggerId,attempts:Number(previous.attempts||0),successfulSteps:Number(previous.successfulSteps||0),consecutiveErrors:0,interruptedRuns:Number(previous.interruptedRuns||0),lastError:null,lastResult:previous.lastResult||null,removedTriggersOnStart:removed};dispatcherSave_(state);return AKORT.Result.success('Alpha.6 reconciliation dispatcher scheduled.',dispatcherPublic_(state));},{lock:true,persistLogs:true});}
  function stopReconciliationDispatcher(){return AKORT.Core.safeRun('PUBLISH_RECONCILIATION_DISPATCHER_STOP',function(){AKORT.EnvironmentGuard.assertDev();var state=dispatcherLoad_()||{startedAt:'',attempts:0,successfulSteps:0,consecutiveErrors:0,interruptedRuns:0},removed=dispatcherDeleteTriggers_();state.enabled=false;state.status='STOPPED_MANUALLY';state.nextRunAt='';state.lastFinishedAt=AKORT.Core.now();state.removedTriggersOnStop=removed;dispatcherSave_(state);return AKORT.Result.success('Alpha.6 reconciliation dispatcher stopped. Reconciliation checkpoint was preserved.',dispatcherPublic_(state));},{lock:true,persistLogs:true});}
  ...
  function reconciliationDispatcherWorker(){
    var state=dispatcherLoad_();
    if(!state||state.enabled!==true){dispatcherDeleteTriggers_();return AKORT.Result.success('Reconciliation dispatcher is disabled.',dispatcherPublic_(state));}
    ...
    if(state.status==='RUNNING'&&state.lastStartedAt&&(!state.lastFinishedAt||String(state.lastFinishedAt)<String(state.lastStartedAt)))state.interruptedRuns=Number(state.interruptedRuns||0)+1;
    dispatcherDeleteTriggers_();var watchdog=dispatcherSchedule_(RECON_DISPATCHER_WATCHDOG_DELAY_MS);state.status='RUNNING';state.lastStartedAt=AKORT.Core.now();state.lastFinishedAt='';state.nextRunAt=watchdog.scheduledFor;state.triggerId=watchdog.triggerId;state.attempts=Number(state.attempts||0)+1;dispatcherSave_(state);
    try{
      var result=continueReconciliation(),fresh=loadRecon_();state.lastFinishedAt=AKORT.Core.now();state.lastResult=dispatcherResultSummary_(result);
      ...
      if(!result||result.ok===false){state.consecutiveErrors=Number(state.consecutiveErrors||0)+1;state.lastError={code:(result&&result.error&&result.error.code)||'STEP_FAILED',message:(result&&result.message)||'Reconciliation step failed.',at:AKORT.Core.now()};if(state.consecutiveErrors>=RECON_DISPATCHER_MAX_CONSECUTIVE_ERRORS)return AKORT.Result.failure('RECONCILIATION_DISPATCHER_STOPPED','Dispatcher stopped after repeated step failures.',dispatcherTerminalState_(state,fresh,'MAX_CONSECUTIVE_ERRORS'));dispatcherDeleteTriggers_();var retry=dispatcherSchedule_(RECON_DISPATCHER_RETRY_DELAY_MS);state.status='RETRY_SCHEDULED';state.nextRunAt=retry.scheduledFor;state.triggerId=retry.triggerId;dispatcherSave_(state);return AKORT.Result.failure('RECONCILIATION_STEP_FAILED','Step failed; dispatcher scheduled a retry.',dispatcherPublic_(state));}
      state.successfulSteps=Number(state.successfulSteps||0)+1;state.consecutiveErrors=0;state.lastError=null;dispatcherDeleteTriggers_();var next=dispatcherSchedule_(RECON_DISPATCHER_NORMAL_DELAY_MS);state.status='SCHEDULED';state.nextRunAt=next.scheduledFor;state.triggerId=next.triggerId;dispatcherSave_(state);return AKORT.Result.success('Reconciliation step completed; next automatic step scheduled.',dispatcherPublic_(state));
    }catch(error){state.lastFinishedAt=AKORT.Core.now();state.consecutiveErrors=Number(state.consecutiveErrors||0)+1;state.lastError={code:error&&error.code?String(error.code):'DISPATCHER_WORKER_ERROR',message:String(error&&error.message||error),stack:error&&error.stack?String(error.stack).slice(0,2000):'',at:AKORT.Core.now()};if(state.consecutiveErrors>=RECON_DISPATCHER_MAX_CONSECUTIVE_ERRORS)return AKORT.Result.failure('RECONCILIATION_DISPATCHER_STOPPED','Dispatcher stopped after repeated worker errors.',dispatcherTerminalState_(state,loadRecon_(),'MAX_CONSECUTIVE_ERRORS'));dispatcherDeleteTriggers_();var retryAfterError=dispatcherSchedule_(RECON_DISPATCHER_RETRY_DELAY_MS);state.status='RETRY_SCHEDULED';state.nextRunAt=retryAfterError.scheduledFor;state.triggerId=retryAfterError.triggerId;dispatcherSave_(state);return AKORT.Result.failure('RECONCILIATION_DISPATCHER_WORKER_ERROR','Dispatcher worker failed; a retry was scheduled.',dispatcherPublic_(state));}
  }
  function resetReconciliation(){var state=dispatcherLoad_();dispatcherDeleteTriggers_();if(state){state.enabled=false;state.status='STOPPED_BY_RECONCILIATION_RESET';state.nextRunAt='';dispatcherSave_(state);}PropertiesService.getScriptProperties().deleteProperty(RECON_STATE_KEY);return AKORT.Result.success('Alpha.6 reconciliation checkpoint cleared. Dispatcher stopped. Temporary reports were not deleted.');}

  function statusSummary(){var ss=getDwh_(),tables={};Object.keys(TABLES).forEach(function(n){var sh=ss.getSheetByName(n);tables[n]=sh?{rows:Math.max(0,sh.getLastRow()-1),columns:sh.getLastColumn()}:null;});return{release:AKORT.Release.manifest(),manifestHash:AKORT.Core.manifestHash(),publishSchemaVersion:AKORT.Release.publishSchemaVersion,dependencySchemaVersion:AKORT.Release.dependencySchemaVersion,runtimeSettings:runtimeSettings_(),serviceTables:tables,reconciliation:publicRecon_(loadRecon_()),dispatcher:dispatcherPublic_(dispatcherLoad_())};}
