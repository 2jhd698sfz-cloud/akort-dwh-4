...
/**
 * Alpha.7.4 Incremental Aggregate Refresh candidate-r3.
 *
 * Goals:
...
AKORT.IncrementalAggregateUpsert = (function () {
  ...
  var RELEASE = '4.0.0-alpha.7.4-candidate-r3';
  var MODE = 'DURABLE_INCREMENTAL_AGGREGATE_REFRESH_PRE_ACCEPTANCE';
  var ACCEPTANCE_STATUS = 'READY_FOR_INDEPENDENT_REVIEW_R3_REGRESSIONS_COMPLETE_PHYSICAL_DISABLED';
  ...
  var TARGET_CONTRACT = 'EXACT_29_VALUE_AND_NUMBER_FORMAT_ONLY_V1';
  var REQUEST_SCHEMA = 'ALPHA74_AGGREGATE_REFRESH_REQUEST_V1';
  var PLAN_SCHEMA = 'ALPHA74_AGGREGATE_PLAN_ARTIFACT_V3';
  var MODEL_SCHEMA = 'ALPHA74_AGGREGATE_MUTATION_MODEL_V3';
  var RUN_SCHEMA = 'ALPHA74_AGGREGATE_REFRESH_RUN_V3';
  ...
  var RUN_HEADERS = Object.freeze([
    ...
    'write_batch_count','write_batch_digest','insert_hint_digest','target_contract_fingerprint'
  ]);
  var QUEUE_HEADERS = Object.freeze([
    ...
    'status','batch_id','attempt_count','last_error_code','updated_at','write_batch_id','forward_attempt_count','rollback_attempt_count'
  ]);
  ...
  var RUN_PHASES = Object.freeze([
    ...
    'MATERIALIZE_MUTATION_QUEUE','VERIFY_MODEL_QUEUE','BUILD_WRITE_BATCHES','BACKUP_CREATE','BACKUP_VERIFY','BACKUP_CAPTURE','APPLY_WRITE_BATCHES',
    'VERIFY_AFFECTED','VERIFY_UNRELATED','VERIFY_GEOMETRY','SUCCESS',
    'ROLLBACK_PRECHECK_AFFECTED','ROLLBACK_PRECHECK_UNRELATED','ROLLBACK_PRECHECK_GEOMETRY','APPLY_ROLLBACK_BATCHES',
    'ROLLBACK_TRIM_GRID','ROLLBACK_VERIFY_AFFECTED','ROLLBACK_VERIFY_UNRELATED','ROLLBACK_VERIFY_GEOMETRY','ROLLBACK_COMPLETE',
  ...
  var DEFAULT_CONFIG = Object.freeze({
    ...
    mutation_batch_rows: 300,
    mutation_batch_payload_bytes: 1500000,
    mutation_batch_series: 100,
  ...
  function validateTargetContractMetadata_(metadata) {
    var m=metadata||{},expected=contract_().Headers.length;
    if(Number(m.max_columns)!==expected)throw error_('ALPHA74_TARGET_COLUMN_CONTRACT_MISMATCH','Target must have exactly the accepted 29 contract columns.',{expected:expected,actual:m.max_columns});
    if(Array.isArray(m.headers)&&JSON.stringify(m.headers)!==JSON.stringify(contract_().Headers))throw error_('ALPHA74_TARGET_HEADERS_MISMATCH','Target headers differ from accepted contract.',{actual:m.headers});
    return {contract:TARGET_CONTRACT,max_columns:expected};
  }
  function assertValueOnlyChunk_(formulas,notes,validations,mergedCount) {
    function any(matrix,predicate){return(matrix||[]).some(function(row){return(row||[]).some(predicate);});}
    if(any(formulas,function(v){return!!text_(v);}))throw error_('ALPHA74_TARGET_NOT_VALUE_ONLY','Target contains formulas; automated row rollback is forbidden.');
    if(any(notes,function(v){return!!text_(v);}))throw error_('ALPHA74_TARGET_NOT_VALUE_ONLY','Target contains notes; automated row rollback is forbidden.');
    if(any(validations,function(v){return v!==null&&v!==undefined;}))throw error_('ALPHA74_TARGET_NOT_VALUE_ONLY','Target contains data validation; automated row rollback is forbidden.');
    if(Number(mergedCount||0)>0)throw error_('ALPHA74_TARGET_NOT_VALUE_ONLY','Target contains merged cells; automated row rollback is forbidden.');
    return true;
  }
  function assertAdvancedValueOnlyGrid_(response) {
    var forbidden=[];
    ((response&&response.sheets)||[]).forEach(function(sheet){(sheet.data||[]).forEach(function(grid){(grid.rowMetadata||[]).forEach(function(meta,index){if(meta&&Object.keys(meta).length)forbidden.push({kind:'ROW_METADATA',index:index,keys:Object.keys(meta)});});(grid.rowData||[]).forEach(function(row,ri){(row.values||[]).forEach(function(cell,ci){var value=cell||{},entered=value.userEnteredValue||{},format=value.userEnteredFormat||{},extra=Object.keys(format).filter(function(k){return k!=='numberFormat';});if(text_(entered.formulaValue))forbidden.push({kind:'FORMULA',row:ri,column:ci});if(text_(value.note))forbidden.push({kind:'NOTE',row:ri,column:ci});if(value.dataValidation)forbidden.push({kind:'DATA_VALIDATION',row:ri,column:ci});if((value.textFormatRuns||[]).length)forbidden.push({kind:'RICH_TEXT',row:ri,column:ci});if(extra.length)forbidden.push({kind:'CELL_FORMAT',row:ri,column:ci,keys:extra});});});});});
    if(forbidden.length)throw error_('ALPHA74_TARGET_NOT_VALUE_ONLY','Target contains physical state outside values and number formats.',{examples:forbidden.slice(0,20),count:forbidden.length});return true;
  }
  function columnA1_(column){var n=Number(column),out='';while(n>0){n-=1;out=String.fromCharCode(65+n%26)+out;n=Math.floor(n/26);}return out;}
  function assertAdvancedValueOnlyRange_(sheet,startRow,count){if(count<=0)return true;var end=Number(startRow)+Number(count)-1,name="'"+sheet.getName().replace(/'/g,"''")+"'",range=name+'!A'+startRow+':'+columnA1_(contract_().Headers.length)+end,response=sheetsApi_().Spreadsheets.get(sheet.getParent().getId(),{ranges:[range],includeGridData:true,fields:'sheets(data(rowMetadata,rowData(values(userEnteredValue,userEnteredFormat,note,dataValidation,textFormatRuns))))'});return assertAdvancedValueOnlyGrid_(response);}
  function assertSheetValueOnlyStructure_(sheet){var reasons=[];if(sheet.getConditionalFormatRules&&sheet.getConditionalFormatRules().length)reasons.push('CONDITIONAL_FORMATS');if(sheet.getBandings&&sheet.getBandings().length)reasons.push('BANDING');if(sheet.getFilter&&sheet.getFilter())reasons.push('BASIC_FILTER');if(typeof SpreadsheetApp!=='undefined'&&SpreadsheetApp.ProtectionType&&sheet.getProtections){if(sheet.getProtections(SpreadsheetApp.ProtectionType.RANGE).length)reasons.push('RANGE_PROTECTION');if(sheet.getProtections(SpreadsheetApp.ProtectionType.SHEET).length)reasons.push('SHEET_PROTECTION');}if(reasons.length)throw error_('ALPHA74_TARGET_NOT_VALUE_ONLY','Target sheet structure contains state outside the reversible contract.',{reasons:reasons});return true;}
  function utf8Bytes_(value){var input=String(value);if(typeof Utilities!=='undefined'&&Utilities.newBlob)return Utilities.newBlob(input).getBytes().length;try{return unescape(encodeURIComponent(input)).length;}catch(ignore){return input.length;}}
  ...
  function physicalTemplateIndex_(templates) {
    var out={},headers=contract_().Headers;
    (templates||[]).forEach(function(raw){var t=clone_(raw||{}),key=requireText_(t,'aggregate_series_key','ALPHA74_PHYSICAL_TEMPLATE_KEY_MISSING');
      if(out[key])throw error_('ALPHA74_PHYSICAL_TEMPLATE_DUPLICATE','Physical template registry contains a duplicate semantic series.',{aggregate_series_key:key});
      if(!Array.isArray(t.number_formats)||t.number_formats.length!==headers.length)throw error_('ALPHA74_PHYSICAL_TEMPLATE_FORMATS_INVALID','Physical template must define exactly one number format for each contract column.',{aggregate_series_key:key,expected:headers.length});
      out[key]=t;
    });
    return out;
  }
  function contractTemplate_(plannedRow,templateIndex) {
    var key=text_(plannedRow.aggregate_series_key),t=(templateIndex||{})[key];
    if(!t)throw error_('ALPHA74_INSERT_TEMPLATE_MISSING','A new physical series requires an immutable contract template.',{aggregate_series_key:key});
    var row={},headers=contract_().Headers,metadata=t.metadata||{};headers.forEach(function(h){row[h]='';});
    row.dataset_code=text_(plannedRow.dataset_code);row.source_name=text_(metadata.source_name||plannedRow.source_name||plannedRow.dataset_code);
    row.frequency=norm_(plannedRow.frequency);row.aggregate_level=norm_(plannedRow.aggregate_level);row.aggregate_name=text_(metadata.aggregate_name||plannedRow.aggregate_name||plannedRow.aggregate_subject_id);
    row.category_id=text_(plannedRow.category_id||metadata.category_id);row.product_group=text_(metadata.product_group||plannedRow.product_group);row.product_name=text_(metadata.product_name||plannedRow.product_name||row.aggregate_name);
    row.value_type=physicalValueType_(plannedRow.value_type);row.index_type=norm_(plannedRow.index_type);
    row._number_formats=t.number_formats.slice();row._physical_types=headers.map(function(h){return physicalType_(row[h]);});row._template_physical_row_number=0;row._contract_template=true;
    plannerRowIdentityCheck_(plannedRow,row);return row;
  }
  function seriesTemplate_(seriesRows, plannedRow, templateIndex) {
    var rows = seriesRows || [];
    if (!rows.length) return contractTemplate_(plannedRow,templateIndex||{});
    var signatures = {};
  ...
  function validateRequest_(request) {
    ...
    if(r.physical_templates!==undefined&&!Array.isArray(r.physical_templates))throw error_('ALPHA74_REQUEST_ARRAY_REQUIRED','physical_templates must be an array when supplied.',{field:'physical_templates'});physicalTemplateIndex_(r.physical_templates||[]);
  ...
  function insertHintDigest_(items){return hash_((items||[]).filter(function(x){return text_(x.action)==='INSERT';}).map(function(x){return text_(x.canonical_row_key)+'|'+String(Number(x.physical_row_hint||0));}).sort());}
  function writeBatchBinding_(batch){return[text_(batch.write_batch_id),String(Number(batch.sequence_no||0)),(batch.series_keys||[]).slice().sort().join('|'),(batch.item_ids||[]).slice().sort().join('|'),String(Number(batch.payload_bytes||0))].join('\u001f');}
  function writeBatchDigest_(batches){return hash_((batches||[]).map(writeBatchBinding_).sort());}
  function mutationSizingPayload_(item){var out=clone_(item);out.write_batch_id='AGWRITE_XXXXXXXXXXXXXXXXXXXXXXXX';return out;}
  function buildSeriesBundles_(items,cfg){var groups={};(items||[]).filter(function(x){return text_(x.action)!=='NOOP';}).forEach(function(item){var key=text_(item.canonical_series_key);groups[key]=groups[key]||[];groups[key].push(item);});
    return Object.keys(groups).sort().map(function(series){var bundle=groups[series].sort(compareBy_(['canonical_row_key','action'])),actions={};bundle.forEach(function(x){actions[text_(x.action)]=true;});if(actions.DELETE&&actions.INSERT)throw error_('ALPHA74_SERIES_BUNDLE_GEOMETRY_CONFLICT','One series bundle cannot combine DELETE and INSERT.',{aggregate_series_key:series});var bytes=utf8Bytes_(canonicalJson_(bundle.map(mutationSizingPayload_)));if(bundle.length>cfg.mutation_batch_rows||bytes>cfg.mutation_batch_payload_bytes)throw error_('ALPHA74_SERIES_BUNDLE_TOO_LARGE','A physical series bundle exceeds one immutable write batch.',{aggregate_series_key:series,rows:bundle.length,payload_bytes:bytes});var priority=actions.DELETE?1:(actions.INSERT?3:2);return{series_key:series,items:bundle,payload_bytes:bytes,priority:priority};});}
  function buildWriteBatches_(items,cfg){var bundles=buildSeriesBundles_(items,cfg).sort(function(a,b){return a.priority!==b.priority?a.priority-b.priority:(a.series_key<b.series_key?-1:a.series_key>b.series_key?1:0);}),batches=[],current=null;
    function commit(){if(!current)return;current.sequence_no=batches.length+1;current.item_ids=current.items.map(function(x){return x.item_id;});current.series_keys=current.series_keys.slice();current.write_batch_id='AGWRITE_'+hash_([current.sequence_no,current.series_keys,current.item_ids,current.payload_bytes]).slice(0,24).toUpperCase();current.items.forEach(function(x){x.write_batch_id=current.write_batch_id;});delete current.items;batches.push(current);current=null;}
    bundles.forEach(function(bundle){var nextRows=(current?current.row_count:0)+bundle.items.length,nextBytes=(current?current.payload_bytes:0)+bundle.payload_bytes,nextSeries=(current?current.series_keys.length:0)+1;if(current&&(nextRows>cfg.mutation_batch_rows||nextBytes>cfg.mutation_batch_payload_bytes||nextSeries>cfg.mutation_batch_series))commit();if(!current)current={row_count:0,payload_bytes:0,series_keys:[],items:[]};current.row_count+=bundle.items.length;current.payload_bytes+=bundle.payload_bytes;current.series_keys.push(bundle.series_key);current.items=current.items.concat(bundle.items);});commit();return batches;}
  function finalizePhysicalPlan_(items,targetLastRow,cfg){var copy=clone_(items||[]),deletes=copy.filter(function(x){return text_(x.action)==='DELETE';}).length,next=Math.max(2,Number(targetLastRow||1)-deletes+1);copy.filter(function(x){return text_(x.action)==='INSERT';}).sort(compareBy_(['canonical_series_key','canonical_row_key'])).forEach(function(x){x.physical_row_hint=next++;});copy.forEach(function(x){if(text_(x.action)!=='INSERT'&&text_(x.action)!=='NOOP'&&Number(x.physical_row_hint||0)<2)throw error_('ALPHA74_PHYSICAL_HINT_MISSING','Existing physical mutation requires an immutable row hint.',{canonical_row_key:x.canonical_row_key});x.item_id='AGITEM_'+hash_([x.action,x.canonical_row_key,x.before_fingerprint,x.after_fingerprint,Number(x.physical_row_hint||0)]).slice(0,24).toUpperCase();x.write_batch_id='';});var batches=buildWriteBatches_(copy,cfg);return{items:copy,write_batches:batches,insert_hint_digest:insertHintDigest_(copy),write_batch_digest:writeBatchDigest_(batches)};}
  function activeDeleteHintsFromRows_(rows){return(rows||[]).filter(function(q){return text_(q.action)==='DELETE'&&text_(q.status)==='APPLIED';}).map(function(q){return Number(q.physical_row_hint||0);}).filter(function(n){return n>=2;}).sort(function(a,b){return a-b;});}
  function activeDeleteHints_(run){return activeDeleteHintsFromRows_(queueRowsForRun_(run,'MUTATION'));}
  ...
  function buildMutationModel_(request, plan, targetRows, physicalContext) {
    var r=validateRequest_(request),scan=canonicalTargetRows_(targetRows,r.definitions,r.membership_snapshot.rule_rows),planned={},deletion={},templates=physicalTemplateIndex_(r.physical_templates||[]),context=physicalContext||{};
    (plan.planned_rows||[]).forEach(function(row){var key=requireText_(row,'aggregate_row_key','ALPHA74_PLANNED_ROW_KEY_MISSING');if(planned[key])throw error_('ALPHA74_DUPLICATE_PLANNED_ROW_KEY','Planner returned a duplicate aggregate row key.',{aggregate_row_key:key});planned[key]=clone_(row);});
    if(r.operation_kind==='REVERSAL')(r.current_effect_row_keys||[]).forEach(function(key){key=text_(key);if(key&&!planned[key])deletion[key]=true;});
    var latest=finalLatestBySeries_(plan,scan,planned,deletion),items=[],touched={};
    Object.keys(planned).sort().forEach(function(key){var calc=planned[key],current=scan.by_key[key]||null,template=current||seriesTemplate_(scan.by_series[calc.aggregate_series_key],calc,templates),after=buildAfterRow_(calc,template,latest.next[calc.aggregate_series_key]===key);after.aggregate_row_key=key;after.aggregate_series_key=calc.aggregate_series_key;var action=current?(rowFingerprint_(current)===rowFingerprint_(after)?'NOOP':'UPDATE'):'INSERT';items.push(mutationItem_(action,key,calc.aggregate_series_key,current,after,{reason:r.operation_kind}));touched[key]=true;});
    Object.keys(deletion).sort().forEach(function(key){var current=scan.by_key[key];if(!current)throw error_('ALPHA74_REVERSAL_TARGET_ROW_MISSING','Evidence-backed reversal row is absent from the pre-operation target.',{aggregate_row_key:key});items.push(mutationItem_('DELETE',key,current.aggregate_series_key,current,null,{reason:'REVERSAL',current_effect_evidence_fingerprint:hash_(r.current_effect_evidence)}));touched[key]=true;});
    Object.keys(latest.affected).sort().forEach(function(series){(scan.by_series[series]||[]).forEach(function(current){if(touched[current.aggregate_row_key]||deletion[current.aggregate_row_key])return;var intended=latest.next[series]===current.aggregate_row_key?1:0;if(Number(current.is_latest_period||0)===intended)return;var after=clone_(current);after.is_latest_period=intended;items.push(mutationItem_('LATEST',current.aggregate_row_key,series,current,after,{reason:'LATEST_INTENT'}));touched[current.aggregate_row_key]=true;});});
    var maxHint=(targetRows||[]).reduce(function(m,x){return Math.max(m,Number(x._physical_row_number||0));},1),physical=finalizePhysicalPlan_(items,Number(context.target_last_row||maxHint),config_(context.config||{}));items=physical.items;
    items.sort(function(a,b){return Number(a.write_batch_id?1:2)-Number(b.write_batch_id?1:2)||(a.canonical_row_key<b.canonical_row_key?-1:a.canonical_row_key>b.canonical_row_key?1:0);});var counts={INSERT:0,UPDATE:0,NOOP:0,DELETE:0,LATEST:0};items.forEach(function(x){counts[x.action]+=1;});
    var model={schema_version:MODEL_SCHEMA,operation_id:r.operation_id,load_id:r.load_id,operation_kind:r.operation_kind,plan_id:text_(plan.plan_id),plan_fingerprint:text_(plan.fingerprint),request_fingerprint:requestFingerprint_(r),items:items,counts:counts,affected_series:Object.keys(latest.affected).sort(),write_batches:physical.write_batches,write_batch_count:physical.write_batches.length,write_batch_digest:physical.write_batch_digest,insert_hint_digest:physical.insert_hint_digest,target_contract:TARGET_CONTRACT};model.item_count=items.length;model.item_digest=mutationSetDigest_(items);model.fingerprint=hash_(model);return model;
  }
  ...
  function nextPhase_(phase){var order=['PREPLAN','SCAN_TARGET','BUILD_PLAN','MATERIALIZE_CALC_QUEUE','CALCULATE_SLICES','VERIFY_CALC_SET','ASSEMBLE_MODEL','MATERIALIZE_MUTATION_QUEUE','VERIFY_MODEL_QUEUE','BUILD_WRITE_BATCHES','BACKUP_CREATE','BACKUP_VERIFY','BACKUP_CAPTURE','APPLY_WRITE_BATCHES','VERIFY_AFFECTED','VERIFY_UNRELATED','VERIFY_GEOMETRY','SUCCESS'];var i=order.indexOf(text_(phase));return i<0||i===order.length-1?text_(phase):order[i+1];}
  ...
  function mutationBinding_(item) { return [text_(item.item_id),text_(item.action),text_(item.canonical_row_key),text_(item.canonical_series_key),text_(item.storage_key),String(Number(item.physical_row_hint||0)),text_(item.write_batch_id),text_(item.before_fingerprint),text_(item.after_fingerprint)].join('\u001f'); }
  ...
  function publishableRowsFromCalculation_(result) {
    ...
    var rows=clone_(result.rows||[]),blocked=rows.filter(function(r){return text_(r.row_type)==='AGGREGATE_RESULT'&&r.publication_allowed!==true;}),partialCategories=rows.filter(function(r){return text_(r.row_type)==='CATEGORY_CONTRIBUTION'&&(r.publication_allowed!==true||text_(r.calculation_status)!=='READY');});
    ...
    if(partialCategories.length)throw error_('ALPHA74_PARTIAL_CONTRIBUTION_SET_FORBIDDEN','Partial category-contribution publication is forbidden because stale physical rows cannot remain.',{row_keys:partialCategories.map(function(r){return r.aggregate_row_key;})});
  ...
  /* ------------------------- Apps Script adapter: candidate-r3 ------------------------- */
  ...
  function ensureSheet_(ss,name,headers){var sheet=ss.getSheetByName(name);if(!sheet)sheet=ss.insertSheet(name);var lastCol=Math.max(1,sheet.getLastColumn()),existing=sheet.getRange(1,1,1,lastCol).getDisplayValues()[0],blank=existing.every(function(v){return!text_(v);});if(blank){sheet.getRange(1,1,1,headers.length).setValues([headers.slice()]);}else{var prefix=headers.slice(0,existing.length);if(JSON.stringify(existing)!==JSON.stringify(prefix)||existing.length>headers.length)throw error_('ALPHA74_SERVICE_SCHEMA_MISMATCH','Service sheet headers differ from Alpha.7.4 candidate-r3.',{sheet:name,expected:headers,actual:existing});if(existing.length<headers.length)sheet.getRange(1,existing.length+1,1,headers.length-existing.length).setValues([headers.slice(existing.length)]);}sheet.setFrozenRows(1);return sheet;}
  ...
  function install(){return withLock_(function(){assertDev_();var sheets=serviceSheets_(),legacyActive=readTable_(sheets.runs,RUN_HEADERS).filter(function(r){return!TERMINAL[text_(r.status)]&&text_(r.schema_version)!==RUN_SCHEMA;});if(legacyActive.length)throw error_('ALPHA74_LEGACY_ACTIVE_RUN_BLOCKS_UPGRADE','A nonterminal pre-r3 run blocks service-schema upgrade.',{run_ids:legacyActive.map(function(r){return r.run_id;})});if(properties_().getProperty(ENABLED_KEY)===null)properties_().setProperty(ENABLED_KEY,'false');if(properties_().getProperty(ACTIVE_RUN_KEY)===null)properties_().setProperty(ACTIVE_RUN_KEY,'');return{ok:true,status:'INSTALLED_DISABLED_CANDIDATE_R3',tables:Object.keys(TABLES).map(function(k){return TABLES[k];}),execution_enabled:enabled_(),physical_publish_writes:false,service_sheets:[sheets.runs.getName(),sheets.queue.getName(),sheets.batches.getName(),sheets.backup.getName()]};});}
  ...
  function targetSheet_(){var sheet=publishSpreadsheet_().getSheetByName(TARGET_SHEET);if(!sheet)throw error_('ALPHA74_TARGET_SHEET_MISSING','Target sheet is missing.');var headerRange=sheet.getRange(1,1,1,contract_().Headers.length),headers=headerRange.getDisplayValues()[0];validateTargetContractMetadata_({max_columns:sheet.getMaxColumns(),headers:headers});assertSheetValueOnlyStructure_(sheet);assertValueOnlyChunk_(headerRange.getFormulas(),headerRange.getNotes(),headerRange.getDataValidations(),headerRange.getMergedRanges().length);return sheet;}
  function scanTargetRows_(sheet,startRow,count){if(count<=0)return[];assertAdvancedValueOnlyRange_(sheet,startRow,count);var range=sheet.getRange(startRow,1,count,contract_().Headers.length),values=range.getValues(),formats=range.getNumberFormats(),formulas=range.getFormulas(),notes=range.getNotes(),validations=range.getDataValidations(),merged=range.getMergedRanges();assertValueOnlyChunk_(formulas,notes,validations,merged.length);return values.map(function(v,i){var o=objectFromRow_(contract_().Headers,v);o._physical_row_number=startRow+i;o._number_formats=formats[i].slice();o._physical_types=v.map(physicalType_);return o;});}
  ...
  function preplanPhase_(run){var request=loadRunRequest_(run),preplan=replayPlanner_(request,[],false),affected=uniqueSorted_(preplanAffectedVisibleSeries_(preplan,request).concat(request.current_effect_visible_series_keys||[]));if(!affected.length){run.status='SUCCESS';run.phase='SUCCESS';properties_().setProperty(ACTIVE_RUN_KEY,'');return saveRun_(run);}var target=targetSheet_();run.plan_id=text_(preplan.plan_id);run.plan_fingerprint=text_(preplan.fingerprint);run.affected_series_json=JSON.stringify(affected);run.target_grid_rows=target.getMaxRows();run.target_last_row=target.getLastRow();run.target_columns=target.getMaxColumns();run.target_contract_fingerprint=hash_(TARGET_CONTRACT);run.target_header_format_fingerprint=hash_(target.getRange(1,1,1,contract_().Headers.length).getNumberFormats()[0]);run.phase='SCAN_TARGET';run.cursor=2;run.status='RUNNING';return saveRun_(run);}
  function scanChunkPartition_(rows,affectedVisibleSeries,runId,batchId,start){var affected={},cache=[],unrelated=emptyAccumulator_();(affectedVisibleSeries||[]).forEach(function(k){affected[text_(k)]=true;});(rows||[]).forEach(function(row){if(affected[visibleSeriesKey_(row)]){var itemId='AGSCAN_'+hash_([runId,storageKey_(row),rowFingerprint_(row)]).slice(0,24).toUpperCase();cache.push({run_id:runId,item_id:itemId,record_kind:'SCAN',sequence_no:Number(start||0),action:'',calculation_id:'',canonical_row_key:'',canonical_series_key:'',visible_series_key:visibleSeriesKey_(row),storage_key:storageKey_(row),physical_row_hint:row._physical_row_number,before_fingerprint:rowFingerprint_(row),after_fingerprint:'',before_json:JSON.stringify(row),after_json:'',input_fingerprint:'',result_artifact_id:'',result_sha256:'',result_fingerprint:'',result_rows_count:0,status:'CACHED',batch_id:batchId,attempt_count:1,last_error_code:'',updated_at:new Date().toISOString(),write_batch_id:'',forward_attempt_count:0,rollback_attempt_count:0});}else unrelated=accumulateToken_(unrelated,storageKey_(row)+'|'+rowFingerprint_(row));});return{cache:cache,unrelated_accumulator:unrelated};}
  function scanPhase_(run,startedMs,cfg){var pause=shouldPause_(startedMs,Date.now(),cfg,'GENERAL');if(pause.pause){run.status='PAUSED';return saveRun_(run);}var affected=JSON.parse(run.affected_series_json||'[]'),sheet=targetSheet_(),last=sheet.getLastRow(),start=Math.max(2,Number(run.cursor||2)),count=Math.min(cfg.scan_chunk_rows,Math.max(0,last-start+1));if(!count){var scans=batchRows_(run.run_id,'SCAN_TARGET').filter(function(b){return text_(b.status)==='COMMITTED';}).sort(compareBy_(['start_cursor'])),acc=emptyAccumulator_();run.target_before_fingerprint=hash_(scans.map(function(b){return text_(b.before_target_fingerprint);}));scans.forEach(function(b){acc=combineAccumulators_(acc,JSON.parse(b.details_json||'{}').unrelated_accumulator||emptyAccumulator_());});run.unrelated_before_fingerprint=hash_(acc);run.phase='BUILD_PLAN';run.cursor=0;return saveRun_(run);}var batchId=batchId_(run.run_id,'SCAN_TARGET','SCAN',[String(start),String(start+count-1)]),existing=batchRows_(run.run_id,'SCAN_TARGET').filter(function(b){return text_(b.batch_id)===batchId&&text_(b.status)==='COMMITTED';})[0];if(existing){run.cursor=start+count;return saveRun_(run);}var rows=scanTargetRows_(sheet,start,count),partition=scanChunkPartition_(rows,affected,run.run_id,batchId,start);upsertQueueRows_(partition.cache);appendBatch_({batch_id:batchId,run_id:run.run_id,phase:'SCAN_TARGET',action:'SCAN',start_cursor:start,end_cursor:start+count-1,item_count:count,request_fingerprint:run.request_fingerprint,before_target_fingerprint:hash_(rows.map(function(r){return String(r._physical_row_number)+'|'+storageKey_(r)+'|'+rowFingerprint_(r);})),after_target_fingerprint:'',status:'COMMITTED',started_at:new Date().toISOString(),finished_at:new Date().toISOString(),execution_id:'',error_code:'',details_json:JSON.stringify({cached_rows:partition.cache.length,unrelated_accumulator:partition.unrelated_accumulator})});run.cursor=start+count;run.heartbeat_at=new Date().toISOString();return saveRun_(run);}
  ...
  function deleteHints_(runId){var run=findRun_(runId).row;return run?activeDeleteHints_(run):[];}
  ...
  function effectiveRow_(item,rollback,activeDeleteHints){var action=text_(item.action),hint=Number(item.physical_row_hint||0);if(rollback===true){if(action==='DELETE'||action==='INSERT')return hint;return adjustedAfterDeletes_(hint,activeDeleteHints);}if(action==='INSERT')return hint;return adjustedAfterDeletes_(hint,activeDeleteHints);}
  ...
  function assignInsertHints_(sheet,items){(items||[]).forEach(function(q){if(text_(q.action)==='INSERT'&&Number(q.physical_row_hint||0)<2)throw error_('ALPHA74_PHYSICAL_HINT_MISSING','INSERT hint must be assigned in the immutable model.',{item_id:q.item_id});});return items;}
  ...
  function physicalBatchRequests_(sheet,items,scan,rollback,activeDeleteHints){var headers=contract_().Headers,requests=[],sheetId=sheet.getSheetId(),deletes=[],updates=[],inserts=[],rollbackInserts=[],active=(activeDeleteHints||[]).slice(),batchDeletes=(items||[]).filter(function(q){return rollback!==true&&text_(q.action)==='DELETE';}).map(function(q){return Number(q.physical_row_hint||0);}),postDelete=uniqueSorted_(active.concat(batchDeletes).map(String)).map(Number).sort(function(a,b){return a-b;});
    (items||[]).forEach(function(q){var original=text_(q.action),action=original,before=q.before_json?JSON.parse(q.before_json):null,after=q.after_json?JSON.parse(q.after_json):null,current=scan.by_key[q.canonical_row_key]||null;if(rollback){if(action==='INSERT')action='DELETE';else if(action==='DELETE')action='INSERT';else action='UPDATE';var tmp=before;before=after;after=tmp;}var rowNumber=(rollback||action==='DELETE'||action==='INSERT')?effectiveRow_(q,rollback,active):adjustedAfterDeletes_(Number(q.physical_row_hint||0),postDelete);if(action==='DELETE'){if(current)deletes.push({row:rowNumber,q:q});}else if(action==='INSERT'){if(rollback&&original==='DELETE')rollbackInserts.push({row:after,q:q,originalRow:rowNumber});else inserts.push({row:after,q:q,rowNumber:rowNumber});}else{if(!current)throw error_('ALPHA74_TARGET_ROW_MISSING','Update target is missing.',{item_id:q.item_id});updates.push({rowNumber:rowNumber,row:after,q:q});}});
    deletes.sort(function(a,b){return b.row-a.row;}).forEach(function(x){requests.push({deleteDimension:{range:{sheetId:sheetId,dimension:'ROWS',startIndex:x.row-1,endIndex:x.row}}});});if(deletes.length)requests.push({appendDimension:{sheetId:sheetId,dimension:'ROWS',length:deletes.length}});
    updates.forEach(function(x){requests.push({updateCells:{start:{sheetId:sheetId,rowIndex:x.rowNumber-1,columnIndex:0},rows:[updateCellsRow_(headers,x.row,true)],fields:'userEnteredValue,userEnteredFormat.numberFormat'}});});
    rollbackInserts.sort(function(a,b){return a.originalRow-b.originalRow;}).forEach(function(x){requests.push({insertDimension:{range:{sheetId:sheetId,dimension:'ROWS',startIndex:x.originalRow-1,endIndex:x.originalRow},inheritFromBefore:true}});requests.push({updateCells:{start:{sheetId:sheetId,rowIndex:x.originalRow-1,columnIndex:0},rows:[updateCellsRow_(headers,x.row,true)],fields:'userEnteredValue,userEnteredFormat.numberFormat'}});});
    if(inserts.length){var maxDest=Math.max.apply(null,inserts.map(function(x){return x.rowNumber;})),needed=maxDest-sheet.getMaxRows();if(needed>0)requests.push({appendDimension:{sheetId:sheetId,dimension:'ROWS',length:needed}});inserts.forEach(function(x){var rowIndex=x.rowNumber-1,templateOriginal=Number(x.row&&x.row._template_physical_row_number||0),templateRow=adjustedAfterDeletes_(templateOriginal,postDelete),contractTemplate=!!(x.row&&x.row._contract_template);if(templateRow>=2)requests.push({copyPaste:{source:{sheetId:sheetId,startRowIndex:templateRow-1,endRowIndex:templateRow,startColumnIndex:0,endColumnIndex:headers.length},destination:{sheetId:sheetId,startRowIndex:rowIndex,endRowIndex:rowIndex+1,startColumnIndex:0,endColumnIndex:headers.length},pasteType:'PASTE_FORMAT',pasteOrientation:'NORMAL'}});requests.push({updateCells:{start:{sheetId:sheetId,rowIndex:rowIndex,columnIndex:0},rows:[updateCellsRow_(headers,x.row,contractTemplate)],fields:contractTemplate?'userEnteredValue,userEnteredFormat.numberFormat':'userEnteredValue'}});});}return requests;}
  ...
  function advanceAttemptBudget_(q,cfg,direction){var rollback=text_(direction).toUpperCase()==='ROLLBACK',field=rollback?'rollback_attempt_count':'forward_attempt_count';if(Number(q[field]||0)>=Number(cfg.max_batch_attempts))throw error_('ALPHA74_RETRY_EXHAUSTED','Durable item exhausted its '+(rollback?'rollback':'forward')+' retry limit.',{item_id:q.item_id,direction:rollback?'ROLLBACK':'FORWARD',attempt_count:q[field],max_attempts:cfg.max_batch_attempts});q[field]=Number(q[field]||0)+1;q.attempt_count=Number(q.attempt_count||0)+1;return q;}
  function markQueueAttempt_(rows,batchId,cfg,direction){(rows||[]).forEach(function(q){advanceAttemptBudget_(q,cfg,direction);q.batch_id=batchId||q.batch_id;});upsertQueueRows_(rows);}
  ...
  function proveWriteBatchPlan_(run){var model=loadModel_(run),expected=model.write_batches||[],rows=batchRows_(run.run_id,'WRITE_PLAN'),actual=rows.map(function(r){var d=JSON.parse(r.details_json||'{}');return{write_batch_id:r.batch_id,sequence_no:Number(r.start_cursor||0),series_keys:d.series_keys||[],item_ids:d.item_ids||[],payload_bytes:Number(d.payload_bytes||0)};});proveExactSet_(expected,actual,writeBatchBinding_,'ALPHA74_WRITE_BATCH_PLAN_MISMATCH');if(Number(run.write_batch_count)!==expected.length||text_(run.write_batch_digest)!==writeBatchDigest_(expected))throw error_('ALPHA74_WRITE_BATCH_PLAN_MISMATCH','Run write-batch metadata differs from immutable model.');return expected;}
  function buildWriteBatchesPhase_(run,startedMs,cfg){var pause=shouldPause_(startedMs,Date.now(),cfg,'GENERAL');if(pause.pause){run.status='PAUSED';return saveRun_(run);}proveMutationQueue_(run);var model=loadModel_(run),batches=model.write_batches||[],start=Number(run.cursor||0),batch=batches[start];if(!batch){proveWriteBatchPlan_(run);run.phase='BACKUP_CREATE';run.cursor=0;return saveRun_(run);}var exists=batchRows_(run.run_id,'WRITE_PLAN').some(function(x){return text_(x.batch_id)===text_(batch.write_batch_id);});if(!exists)appendBatch_({batch_id:batch.write_batch_id,run_id:run.run_id,phase:'WRITE_PLAN',action:'BUNDLE',start_cursor:batch.sequence_no,end_cursor:batch.sequence_no,item_count:batch.item_ids.length,request_fingerprint:run.request_fingerprint,before_target_fingerprint:'',after_target_fingerprint:'',status:'PLANNED',started_at:new Date().toISOString(),finished_at:'',execution_id:'',error_code:'',details_json:JSON.stringify({item_ids:batch.item_ids,series_keys:batch.series_keys,payload_bytes:batch.payload_bytes})});run.cursor=start+1;return saveRun_(run);}
  function queueIndex_(rows){var out={};(rows||[]).forEach(function(x){out[text_(x.item_id)]=x;});return out;}
  function batchQueueItems_(batch,index){return(batch.item_ids||[]).map(function(id){var q=index[text_(id)];if(!q)throw error_('ALPHA74_WRITE_BATCH_PLAN_MISMATCH','Write batch references an absent queue item.',{item_id:id,write_batch_id:batch.write_batch_id});return q;});}
  function processMutationBatchesPhase_(run,startedMs,cfg){var pause=shouldPause_(startedMs,Date.now(),cfg,'WRITE');if(pause.pause){run.status='PAUSED';return saveRun_(run);}var queue=proveMutationQueue_(run),plans=proveWriteBatchPlan_(run);if(text_(run.backup_fingerprint)!==text_(run.target_before_fingerprint))throw error_('ALPHA74_BACKUP_NOT_VERIFIED','Mutation is forbidden until backup is verified.');var idx=queueIndex_(queue),plan=null,items=[];for(var i=0;i<plans.length;i+=1){var candidate=batchQueueItems_(plans[i],idx).filter(function(q){return text_(q.status)!=='APPLIED';});if(candidate.length){plan=plans[i];items=batchQueueItems_(plan,idx);break;}}if(!plan){run.phase='VERIFY_AFFECTED';return saveRun_(refreshAppliedCount_(run));}var request=loadRunRequest_(run),sheet=targetSheet_(),active=activeDeleteHintsFromRows_(queue),scan=targetedCurrent_(sheet,items,request,false,active),attempted=attemptedItemIds_(run.run_id,'APPLY_WRITE_BATCHES','BUNDLE'),before=[],after=[],third=[];items.forEach(function(item){var state=classifyItemState_(item,scan.by_key[item.canonical_row_key]||null);if(state==='BEFORE')before.push(item);else if(state==='AFTER')after.push(item);else third.push(item);});if(third.length)throw error_('ALPHA74_PREWRITE_DRIFT','Mutation contains third state.',{item_ids:third.map(function(x){return x.item_id;})});var unexplained=after.filter(function(x){return!attempted[x.item_id];});if(unexplained.length)throw error_('ALPHA74_PREAPPLIED_WITHOUT_ATTEMPT','After-state exists without durable prior batch.',{item_ids:unexplained.map(function(x){return x.item_id;})});if(after.length)updateQueueStatuses_(after,'APPLIED','RECOVERED_BY_READBACK','');if(!before.length)return saveRun_(refreshAppliedCount_(run));if(before.length!==items.length)throw error_('ALPHA74_AMBIGUOUS_WRITE','One immutable series bundle is in a mixed state.',{write_batch_id:plan.write_batch_id});items=before;var attemptNo=Math.max.apply(null,items.map(function(x){return Number(x.forward_attempt_count||0);}).concat([0]))+1,executionId=batchId_(run.run_id,'APPLY_WRITE_BATCHES',plan.write_batch_id,[String(attemptNo)]),pre=classifyBatchOutcome_(items,targetedCurrent_(sheet,items,request,false,active).by_key);if(pre.status!=='RETRY_SAFE')throw error_('ALPHA74_PREWRITE_DRIFT','Batch is not in exact before-state.',{write_batch_id:plan.write_batch_id,outcome:pre});var requests=physicalBatchRequests_(sheet,items,scan,false,active),batch={batch_id:executionId,run_id:run.run_id,phase:'APPLY_WRITE_BATCHES',action:'BUNDLE',start_cursor:plan.sequence_no,end_cursor:plan.sequence_no,item_count:items.length,request_fingerprint:run.request_fingerprint,before_target_fingerprint:targetedFingerprint_(items,scan.by_key),after_target_fingerprint:'',status:'PREPARED',started_at:new Date().toISOString(),finished_at:'',execution_id:executionId,error_code:'',details_json:JSON.stringify({write_batch_id:plan.write_batch_id,item_ids:items.map(function(x){return x.item_id;})})};appendBatch_(batch);markQueueAttempt_(items,executionId,cfg,'FORWARD');var thrown=null;try{sheetsApi_().Spreadsheets.batchUpdate({requests:requests},publishSpreadsheet_().getId());}catch(e){thrown=e;}SpreadsheetApp.flush();var afterActive=uniqueSorted_(active.concat(items.filter(function(x){return text_(x.action)==='DELETE';}).map(function(x){return String(x.physical_row_hint);}))).map(Number),afterScan=targetedCurrent_(targetSheet_(),items,request,false,afterActive),outcome=classifyBatchOutcome_(items,afterScan.by_key);batch.after_target_fingerprint=targetedFingerprint_(items,afterScan.by_key);batch.finished_at=new Date().toISOString();batch.status=outcome.status;batch.error_code=thrown?text_(thrown.message||thrown):'';appendBatch_(batch);if(outcome.status==='COMMITTED'){updateQueueStatuses_(items,'APPLIED',executionId,'');return saveRun_(refreshAppliedCount_(run));}if(outcome.status==='RETRY_SAFE')return saveRun_(run);throw error_('ALPHA74_AMBIGUOUS_WRITE','Target read-back is mixed or unknown.',{write_batch_id:plan.write_batch_id,outcome:outcome});}
  function processRollbackBatchesPhase_(run,startedMs,cfg){var pause=shouldPause_(startedMs,Date.now(),cfg,'WRITE');if(pause.pause){run.status='PAUSED';return saveRun_(run);}var queue=proveMutationQueue_(run),plans=proveWriteBatchPlan_(run).slice().sort(function(a,b){return Number(b.sequence_no)-Number(a.sequence_no);}),idx=queueIndex_(queue),plan=null,items=[];for(var i=0;i<plans.length;i+=1){var candidate=batchQueueItems_(plans[i],idx).filter(function(q){return text_(q.status)==='APPLIED';});if(candidate.length){plan=plans[i];items=candidate;break;}}if(!plan){run.phase='ROLLBACK_TRIM_GRID';return saveRun_(run);}var request=loadRunRequest_(run),sheet=targetSheet_(),active=activeDeleteHintsFromRows_(queue),scan=targetedCurrent_(sheet,items,request,true,active),restored=[],needs=[],third=[];items.forEach(function(item){var state=classifyItemState_(item,scan.by_key[item.canonical_row_key]||null);if(state==='BEFORE')restored.push(item);else if(state==='AFTER')needs.push(item);else third.push(item);});if(third.length)throw error_('ALPHA74_ROLLBACK_CONFLICT','Rollback would overwrite a third state.',{item_ids:third.map(function(x){return x.item_id;})});if(restored.length)updateQueueStatuses_(restored,'ROLLED_BACK','ALREADY_RESTORED','');if(!needs.length)return saveRun_(run);if(needs.length!==items.length)throw error_('ALPHA74_ROLLBACK_CONFLICT','One immutable series bundle is in a mixed rollback state.',{write_batch_id:plan.write_batch_id});items=needs;var attemptNo=Math.max.apply(null,items.map(function(x){return Number(x.rollback_attempt_count||0);}).concat([0]))+1,executionId=batchId_(run.run_id,'APPLY_ROLLBACK_BATCHES',plan.write_batch_id,[String(attemptNo)]),pre=classifyRollbackOutcome_(items,scan.by_key);if(pre.status!=='RETRY_SAFE')throw error_('ALPHA74_ROLLBACK_CONFLICT','Rollback batch is not in operation after-state.',{write_batch_id:plan.write_batch_id,outcome:pre});var requests=physicalBatchRequests_(sheet,items,scan,true,active),batch={batch_id:executionId,run_id:run.run_id,phase:'APPLY_ROLLBACK_BATCHES',action:'ROLLBACK_BUNDLE',start_cursor:plan.sequence_no,end_cursor:plan.sequence_no,item_count:items.length,request_fingerprint:run.request_fingerprint,before_target_fingerprint:targetedFingerprint_(items,scan.by_key),after_target_fingerprint:'',status:'PREPARED',started_at:new Date().toISOString(),finished_at:'',execution_id:executionId,error_code:'',details_json:JSON.stringify({write_batch_id:plan.write_batch_id,item_ids:items.map(function(x){return x.item_id;})})};appendBatch_(batch);markQueueAttempt_(items,executionId,cfg,'ROLLBACK');var thrown=null;try{sheetsApi_().Spreadsheets.batchUpdate({requests:requests},publishSpreadsheet_().getId());}catch(e){thrown=e;}SpreadsheetApp.flush();var restoredDelete={};items.filter(function(x){return text_(x.action)==='DELETE';}).forEach(function(x){restoredDelete[Number(x.physical_row_hint||0)]=true;});var afterActive=active.filter(function(h){return!restoredDelete[h];}),afterScan=targetedCurrent_(targetSheet_(),items,request,true,afterActive),outcome=classifyRollbackOutcome_(items,afterScan.by_key);batch.after_target_fingerprint=targetedFingerprint_(items,afterScan.by_key);batch.finished_at=new Date().toISOString();batch.status=outcome.status;batch.error_code=thrown?text_(thrown.message||thrown):'';appendBatch_(batch);if(outcome.status==='RESTORED'){updateQueueStatuses_(items,'ROLLED_BACK',executionId,'');return saveRun_(run);}if(outcome.status==='RETRY_SAFE')return saveRun_(run);throw error_('ALPHA74_ROLLBACK_CONFLICT','Rollback read-back is ambiguous.',{write_batch_id:plan.write_batch_id,outcome:outcome});}
  function processMutationPhase_(){throw error_('ALPHA74_DEPRECATED_ACTION_BATCH_PATH','Action-sliced batching is disabled in candidate-r3.');}
  ...
  function rollbackPrecheckGeometryPhase_(run){var sheet=targetSheet_();if(text_(run.target_contract_fingerprint)!==hash_(TARGET_CONTRACT)||sheet.getMaxColumns()!==contract_().Headers.length||hash_(sheet.getRange(1,1,1,contract_().Headers.length).getNumberFormats()[0])!==text_(run.target_header_format_fingerprint))throw error_('ALPHA74_ROLLBACK_CONFLICT','Rollback geometry/header precheck failed.');run.phase='APPLY_ROLLBACK_BATCHES';run.rollback_cursor=0;return saveRun_(run);}
  ...
  function statusSummary(){return{release_candidate:RELEASE,version:VERSION,mode:MODE,acceptance_status:ACCEPTANCE_STATUS,accepted_base_tag:ACCEPTED_BASE_TAG,accepted_base_commit:ACCEPTED_BASE_COMMIT,target_sheet:TARGET_SHEET,target_contract:TARGET_CONTRACT,service_tables:clone_(TABLES),execution_enabled_default:false,operation_kinds:Object.keys(OPERATION_KINDS),actions:Object.keys(ACTIONS),timeout_model:'DURABLE_CALCULATION_AND_SERIES_BUNDLE_QUEUE_CHUNKED_VERIFICATION_READBACK_RECOVERY',calculation_identity:'calculation_id',queue_binding:'EXACT_MODEL_PHYSICAL_HINT_WRITE_BATCH_BINDING',write_batching:'INDIVISIBLE_CANONICAL_SERIES_BUNDLES',new_series_support:'IMMUTABLE_PHYSICAL_TEMPLATE_REGISTRY',partial_contribution_policy:'FAIL_CLOSED',regular_pipeline_enabled:false,physical_writes_during_package_install:false};}
