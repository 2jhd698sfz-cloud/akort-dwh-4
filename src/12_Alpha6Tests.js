...
AKORT.Alpha6Tests = (function(){
  ...
  function runSmokeTest(){return AKORT.Core.safeRun('ALPHA6_SMOKE_TEST',function(){AKORT.EnvironmentGuard.assertDev();var T=AKORT.IncrementalPublish.Test,tests=[];
    ...
    tests.push(test_('operation_phase_contract_consistent',function(){var phases=AKORT.Release.operationPhases||[];if(phases.indexOf('UPDATE_AGGREGATES')<0||phases.indexOf('PLAN_AGGREGATE_IMPACT')>=0)throw new Error('Release phases do not match Operation Engine');return phases;}));
    ...
    tests.push(test_('aggregate_replay_frontier',function(){var frontier={};frontier[T.frontierKey('weekly','AKORT_WEEKLY','2025-12-28')]=true;frontier[T.frontierKey('weekly','AKORT_WEEKLY','2026-01-04')]=true;var x=T.expandAffected([{frequency:'weekly',datasetCode:'AKORT_WEEKLY',categoryId:'C',valueType:'розница',indexType:'',period:'2025-12-28'}],{aggregateFrontier:frontier}),future=x.aggregates.filter(function(c){return String(c.period).indexOf('2027-')===0;});if(future.length)throw new Error('Replay frontier leaked future periods');if(!x.aggregates.length)throw new Error('Frontier removed source/direct periods');return{aggregateCount:x.aggregates.length,latest:x.aggregates.map(function(c){return c.period;}).sort().slice(-3)};}));
