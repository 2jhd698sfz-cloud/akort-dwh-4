...
/**
 * v4.0.0-alpha.6.2.1 Incremental Publish.
...
function v310HasValue_(v) { return v !== '' && v !== null && v !== undefined && !isNaN(Number(v)); }
  ...
  var RECON_DISPATCHER_STATE_KEY='AKORT_ALPHA6_RECONCILIATION_DISPATCHER_STATE_V5';
  var RECON_DISPATCHER_CONTROL_KEY='AKORT_ALPHA6_RECONCILIATION_DISPATCHER_CONTROL_V3';
  ...
  function replayImpactPreview_(state,group){var plan=replayPlan_(state,group),counts={},invalid=0;(plan.aggregates||[]).forEach(function(c){counts[c.indexType]=(counts[c.indexType]||0)+1;if(!c.indexType)invalid++;});var preview={loadId:group.loadId,aggregateCombos:(plan.aggregates||[]).length,indexTypes:counts,invalidIndexTypes:invalid,frontierMode:ALPHA6_SCOPE.replayFrontierMode,frontierKeys:Number(state.aggregateFrontierCount||0),executionStatus:ALPHA6_SCOPE.aggregateExecutionStatus};if(invalid)throw AKORT.Core.error('AGGREGATE_IMPACT_INDEX_TYPE_INVALID','Aggregate impact preview contains an empty index type.',preview);state.aggregateImpactPreview=(state.aggregateImpactPreview||[]).filter(function(old){return String(old.loadId)!==String(group.loadId);});state.aggregateImpactPreview.push(preview);var sh=SpreadsheetApp.openById(state.controlId).getSheetByName('REPLAY_GROUPS');if(sh)sh.getRange(state.replayIndex+2,6).setValue(AKORT.Core.safeJson(preview));return preview;}
  ...
  function replayStep_(state){
    ...
    if(stage==='IMPACT_PREVIEW')return completeReplayGroup_(state,groups,group,replayImpactPreview_(state,group));
  ...
  function reconciliationDispatcherWorker(){
    ...
    try{
      ...
      try{
        ...
        if(fresh&&(fresh.status==='SUCCESS'||fresh.status==='FAILED')){dispatcherApplyProgress_(state,beforeFingerprint,fresh);return fresh.status==='SUCCESS'?AKORT.Result.success('Alpha.6.2.1 reconciliation completed; dispatcher stopped.',dispatcherTerminalState_(state,fresh,'RECONCILIATION_'+fresh.status)):AKORT.Result.failure('PUBLISH_RECONCILIATION_FAILED','Alpha.6.2.1 reconciliation failed; dispatcher stopped.',dispatcherTerminalState_(state,fresh,'RECONCILIATION_'+fresh.status));}
