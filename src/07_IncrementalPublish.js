...
function v310HasValue_(v) { return v !== '' && v !== null && v !== undefined && !isNaN(Number(v)); }
  ...
  function copyPublishBackup_(options){
    options=options||{};
    AKORT.EnvironmentGuard.assertDev();
    var config=AKORT.Config.load({includeSystemSettings:false});
    var source=DriveApp.getFileById(config.resources.publishSpreadsheetId);
    var folder=DriveApp.getFolderById(options.folderId||config.resources.testResultsFolderId);
    var stamp=Utilities.formatDate(new Date(),'Europe/Moscow','yyyyMMdd_HHmmss');
    var name=String(options.name||('AKORT_PUBLISH_ALPHA6_BACKUP_'+stamp));
    var copy=source.makeCopy(name,folder);
    return{backupId:copy.getId(),backupName:copy.getName(),backupUrl:copy.getUrl(),sourcePublishId:config.resources.publishSpreadsheetId,createdAt:AKORT.Core.now()};
  }
  function createPublishBackup(options){return AKORT.Core.safeRun('INCREMENTAL_PUBLISH_BACKUP',function(){return AKORT.Result.success('DEV Publish backup created.',copyPublishBackup_(options));},{lock:true,persistLogs:true});}
...
function continueReconciliation(){return AKORT.Core.safeRun('PUBLISH_RECONCILIATION_CONTINUE',function(){
  ...
  function alpha74BuildAcceptedAggregateRows_(combos) {
    ...
    (combos||[]).forEach(function(combo){
      var normalized={
        frequency:String(combo&&combo.frequency||'').toLowerCase(),
        datasetCode:String(combo&&(combo.datasetCode||combo.dataset_code)||''),
        period:combo&&(combo.period||combo.period_start),
        valueType:combo&&(combo.valueType||combo.value_type),
        indexType:combo&&(combo.indexType||combo.index_type||'')
      };
      var key=v310ComboKey_(normalized);
      if(key)comboIndex[key]=true;
    });
    var rows=v310BuildAggregateRowsForCombos_(comboIndex);
    return clone_(reconNormalizeAggregateDates_(rows));
  }
