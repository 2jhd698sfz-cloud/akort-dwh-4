...
AKORT.Alpha74UpsertTests = (function () {
  var SUITE = 'ALPHA74_INCREMENTAL_AGGREGATE_UPSERT_CANDIDATE_R2';
  ...
  function runPureTests() {
    ...
    test('IU74-001','public version/status metadata',function(){var s=AKORT.IncrementalAggregateUpsert.statusSummary();assert_(s.version==='4.0-incremental-aggregate-refresh-2');assert_(s.execution_enabled_default===false);assert_(s.actions.indexOf('INSERT')>=0);});
    ...
    test('IU74-013','planner is planning-only in candidate-r2',function(){var seen=[];var fp={planRevision:function(i){seen.push(i.execute_calculator);return{ok:true,status:'SUCCESS',plan_id:'P',fingerprint:'F',calculator_batches:[],calculator_shared_input:{},planned_rows:[],latest_intents:[],latest_conflicts:[]};},planReversal:function(i){return this.planRevision(i);}};T.replayPlanner(request_(d,'REVISION'),[],false,fp);assert_(seen[0]===false);});
    ...
    test('IU74-050','service table schemas and phase progression',function(){assert_(AKORT.IncrementalAggregateUpsert.RunHeaders.indexOf('heartbeat_at')>=0);assert_(AKORT.IncrementalAggregateUpsert.QueueHeaders.indexOf('result_artifact_id')>=0);assert_(T.nextPhase('APPLY_INSERT')==='APPLY_LATEST');});
  ...
  return Object.freeze({Version:'4.0-alpha74-upsert-tests-2',runPureTests:runPureTests});
})();
