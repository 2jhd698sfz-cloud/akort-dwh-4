...
AKORT.Baseline = (function () {
  ...
  function createReport_(config) {
    ...
    var report = SpreadsheetApp.create(
      'AKORT_BASELINE_' + config.baselineLabel + '_' + stamp
    );
    ...
    [
      REPORT_SHEETS.inventory,
      REPORT_SHEETS.chunks,
      REPORT_SHEETS.tests,
      REPORT_SHEETS.log
    ].forEach(function (name) {
      report.insertSheet(name);
    });

    report
      .getSheetByName(REPORT_SHEETS.manifest)
      .getRange('A1:B1')
      .setValues([[
        'key',
        'value'
    ...
    report
      .getSheetByName(REPORT_SHEETS.inventory)
      .getRange('A1:O1')
      .setValues([[
        'resource_code',
        'spreadsheet_name',
    ...
    report
      .getSheetByName(REPORT_SHEETS.chunks)
      .getRange('A1:G1')
      .setValues([[
        'resource_code',
        'sheet_name',
    ...
    report
      .getSheetByName(REPORT_SHEETS.tests)
      .getRange('A1:H1')
      .setValues([[
        'test_id',
        'severity',
    ...
    report
      .getSheetByName(REPORT_SHEETS.log)
      .getRange('A1:F1')
      .setValues([[
        'timestamp',
        'level',
    ...
    file.moveTo(
      DriveApp.getFolderById(
        config.resources.testResultsFolderId
      )
    );
  ...
  function initializeState_(report, config) {
    var state = {
      ...
      targets: [
        {
          code: 'DWH',
          spreadsheetId: config.resources.dwhSpreadsheetId
        },
        {
          code: 'PUBLISH',
          spreadsheetId: config.resources.publishSpreadsheetId
        }
      ],
    ...
    var manifest = report.getSheetByName(
      REPORT_SHEETS.manifest
    );
    ...
    writeKeyValue_(
      ...
      'dwh_name',
      SpreadsheetApp
        .openById(config.resources.dwhSpreadsheetId)
        .getName()
    );
    ...
    writeKeyValue_(
      ...
      'publish_name',
      SpreadsheetApp
        .openById(config.resources.publishSpreadsheetId)
        .getName()
    );
  ...
  function countTruthy_(values, columnIndex) {
    ...
    return values.reduce(function (count, row) {
      ...
      return count + (isTruthy ? 1 : 0);
    }, 0);
  }

  function finishSheet_(report, state, sheet, headers) {
    var chunks = report.getSheetByName(
      REPORT_SHEETS.chunks
    );
    ...
    var finalHash = hashText_(
      hashes.join('|')
    );

    var inventory = report.getSheetByName(
      REPORT_SHEETS.inventory
    );
  ...
  function process_(state, config, logger) {
    var report = SpreadsheetApp.openById(
      state.reportId
    );
    ...
    while (
      state.targetIndex < state.targets.length &&
      ...
      var latestColumn = headers.indexOf(
        'is_latest_period'
      );

      if (latestColumn < 0) {
        latestColumn = headers.indexOf(
          'is_latest'
        );
      }
      ...
      appendRows_(
        report.getSheetByName(REPORT_SHEETS.chunks),
        [[
          target.code,
          sheet.getName(),
          state.nextRow,
      ...
      state.latestTrueCount += countTruthy_(
        values,
        latestColumn
      );
    ...
    appendRows_(
      report.getSheetByName(REPORT_SHEETS.log),
      logger.rows()
    );
  ...
  function updateManifestStatus_(report, value) {
    var sheet = report.getSheetByName(
      REPORT_SHEETS.manifest
    );
  ...
  function finalize_(report, state, config) {
    ...
    function latestFor_(name) {
      var row = rows.filter(function (item) {
        return item[idx.sheet_name] === name;
      })[0];

      return row
        ? Number(
            row[idx.latest_true_count] || 0
          )
        : -1;
    }

    function latestTotalFor_(names) {
      return rows
        .filter(function (row) {
          return names.indexOf(
            row[idx.sheet_name]
    ...
    var tests = [
      testRow_(
        ...
        'Total current RAW observation rows (is_latest = 1)'
      ),

      testRow_(
        'BASELINE_PUBLISH_MAIN_ROWS',
        'CRITICAL',
        expected.publishMainRows,
      ...
      testRow_(
        'LATEST_WEEKLY_ROWS',
        'CRITICAL',
        expected.weeklyLatestRows,

        latestFor_(
          'PUBLISH_PRICES_WEEKLY'
        ),

        'Weekly latest-period flags'
      ),

      testRow_(
        'LATEST_MONTHLY_ROWS',
        'CRITICAL',
        expected.monthlyLatestRows,

        latestFor_(
          'PUBLISH_PRICES_MONTHLY'
        ),

        'Monthly latest-period flags'
      ),

      testRow_(
        'LATEST_AGGREGATE_ROWS',
        'CRITICAL',
        expected.aggregateLatestRows,

        latestFor_(
          'PUBLISH_PRICE_AGGREGATES'
        ),

        'Aggregate latest-period flags'
      )
    ];
    ...
    appendRows_(
      report.getSheetByName(REPORT_SHEETS.tests),
      tests
    );

    var overall = tests.every(function (row) {
      return row[2] === 'PASS';
    })
      ? 'PASS'
      : 'FAIL';
    ...
    writeKeyValue_(
      report.getSheetByName(REPORT_SHEETS.manifest),
      'completed_at',
      state.completedAt
    );
    ...
    PropertiesService
      .getScriptProperties()
      .deleteProperty(STATE_KEY);

    return overall === 'PASS'
      ? AKORT.Result.success(
          'Baseline snapshot completed and all alpha.1 controls passed.',
          {
            reportId: report.getId(),
            reportUrl: report.getUrl(),
            status: overall
          }
        )
      : AKORT.Result.failure(
          'BASELINE_MISMATCH',
          'Baseline snapshot completed with failed controls.',
          {
            reportId: report.getId(),
            reportUrl: report.getUrl(),
            status: overall
  ...
  function start() {
    ...
    try {
      ...
      if (existing) {
        throw new Error(
          'An alpha.1 baseline operation is already active. Continue or reset it.'
        );
      }
      ...
      var logger = AKORT.Logger.create(
        'BASELINE_START'
      );

      logger.info(
        'Baseline snapshot started',
        {
          reportId: report.getId()
        }
      );
    ...
    try {
      ...
      if (!state) {
        return AKORT.Result.failure(
          'NO_ACTIVE_BASELINE',
          'No active baseline operation was found.'
        );
      }
  ...
  function status_() {
    ...
    var lastReportId = PropertiesService
      .getScriptProperties()
      .getProperty(LAST_REPORT_KEY);
  ...
  function reset() {
    ...
    return AKORT.Result.success(
      'Active alpha.1 baseline state cleared. Existing report files were not deleted.',
      status_()
    );
  }
